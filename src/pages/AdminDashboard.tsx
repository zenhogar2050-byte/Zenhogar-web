...
export default function AdminDashboard() {
  ...
  const totalRevenue = orders
    .filter(o => o.status === 'delivered' || (o.status === 'pending' && o.type === 'order'))
    .reduce((acc, curr) => acc + (curr.total || curr.cart?.total || 0), 0);
  ...
  const filteredOrders = orders
    ...
    .filter(o => {
      ...
      return (
        o.customer.nombre.toLowerCase().includes(search) ||
        o.customer.apellido.toLowerCase().includes(search) ||
        o.customer.telefono.includes(search) ||
    ...
    <div className="min-h-screen bg-stone-50">
      ...
      <aside className="fixed left-0 top-0 bottom-0 w-64 bg-stone-900 text-white p-6 hidden lg:block z-50">
        <div className="flex items-center gap-3 mb-10">
          <img src="/assets/logo/favicon.png" className="w-8 h-8 object-contain" alt="Logo" />
        ...
        <div className="p-4 lg:p-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
            ...
            <StatCard 
              ...
              value={orders.filter(o => o.type === 'order').length} 
              icon={<Package className="w-6 h-6" />}
              color="blue"
            />
            <StatCard 
              label="Carritos Abandonados" 
              value={orders.filter(o => o.type === 'abandoned').length} 
              icon={<Trash2 className="w-6 h-6" />}
              color="orange"
            />
            <StatCard 
              label="Pendientes de Envío" 
              value={orders.filter(o => o.status === 'pending' && o.type === 'order').length} 
                      ...
                      <tr key={order.id} className="hover:bg-stone-50/50 transition-colors group">
                        <td className="px-6 py-5">
                          <div className="flex items-center gap-3">
                            <div className="w-10 h-10 rounded-full bg-stone-100 flex items-center justify-center text-stone-500 font-bold group-hover:bg-emerald-50 group-hover:text-emerald-600 transition-colors">
                              {order.customer.nombre[0]}
                            </div>
                            <div>
                              <div className="font-bold text-stone-900 leading-tight">{order.customer.nombre} {order.customer.apellido}</div>
                        ...
                        <td className="px-6 py-5">
                          <div className="max-w-xs">
                            <p className="text-xs text-stone-600 line-clamp-2">
                              {order.cart?.items?.map((i: any) => `${i.quantity}x ${i.name}`).join(', ') || 'N/A'}
                            </p>
                          </div>
                        </td>
                        ...
                        <td className="px-6 py-5 text-right">
                          <div className="flex justify-end gap-2">
                             <a 
                              href={`https://wa.me/${order.customer.telefono.replace(/\+/g, '')}`} 
                              target="_blank" 
                              rel="noreferrer"
